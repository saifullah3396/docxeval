...
for eval_arg in "${eval_args[@]}"; do
    ...
    for explainer_arg in "${explainer_args[@]}"; do
        ...
        script="python src/docxeval/explanation_analyzer/${TASK}.py --project-name "$PROJECT_NAME" --exp-name "$EXP_NAME" --output-dir ../outputs/ $eval_arg $explainer_arg --explanations-dir ${EXPLAINER_DIR} --mode summarize_explanations $@"
